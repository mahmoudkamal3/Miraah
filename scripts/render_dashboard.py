...
.grid{display:grid;grid-template-columns:1.4fr 1fr;gap:14px}.panel{background:linear-gradient(160deg,var(--surface-raised),var(--surface));border:1px solid var(--line);border-radius:18px;padding:17px;box-shadow:var(--shadow-soft);min-width:0}.panel-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:14px}.panel-head h4{margin:0;font-size:14px}.panel-head select{width:auto;max-width:290px;padding:8px}.chart{height:320px;position:relative}.chart svg{width:100%;height:100%;overflow:visible}.legend{display:flex;gap:16px;color:var(--muted);font-size:11px;flex-wrap:wrap}.dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-inline-end:5px}.tooltip{position:fixed;display:none;background:var(--surface-tooltip);border:1px solid var(--border-strong);border-radius:9px;padding:8px 10px;font-size:11px;pointer-events:none;z-index:50;box-shadow:var(--shadow)}.radar-note{font-size:11px;color:var(--muted);line-height:1.6;margin-top:5px}.happiness{display:grid;grid-template-columns:1fr 1fr;gap:12px}.happy-card{background:var(--surface-card);border:1px solid var(--line);border-radius:14px;padding:14px}.happy-score{display:flex;align-items:end;justify-content:space-between}.happy-score strong{font-size:34px;direction:ltr}.happy-score span{font-size:11px;color:var(--muted)}.factor{margin-top:10px}.factor-row{display:flex;justify-content:space-between;font-size:11px;margin-bottom:4px}.track{height:6px;border-radius:8px;background:var(--track);overflow:hidden}.fill{height:100%;border-radius:8px}.table-wrap{overflow:auto;max-height:520px;border-radius:12px;border:1px solid var(--line)}table{width:100%;border-collapse:collapse;font-size:12px;min-width:750px}th{position:sticky;top:0;background:var(--table-head);color:var(--table-head-text);text-align:start;padding:11px;z-index:1}td{padding:10px 11px;border-top:1px solid var(--table-row)}tr:hover td{background:var(--table-head)88}.rank{font-weight:800;color:var(--rank-muted)}.barcell{min-width:150px}.mini-bar{display:flex;align-items:center;gap:7px}.mini-track{width:90px;height:6px;background:var(--track-soft);border-radius:7px;overflow:hidden}.mini-fill{height:100%;background:linear-gradient(90deg,var(--brand-blue),var(--brand-cyan));border-radius:7px}.source{margin:20px 2px 4px;color:var(--muted);font-size:11px;line-height:1.7}.empty{height:100%;display:grid;place-items:center;color:var(--muted);text-align:center;padding:16px}.swap-flash{animation:swapFlash .45s ease}@keyframes swapFlash{0%{transform:scale(.985);opacity:.65}100%{transform:scale(1);opacity:1}}
...
const state={a:null,b:null,queryA:'',queryB:'',trend:'life_expectancy',rank:'gdp_ppp',lang:localStorage.getItem('miraahLang')||localStorage.getItem('countryMirrorLang')||'ar',swapNonce:0,openA:false,openB:false,activeIndexA:-1,activeIndexB:-1,matchesA:[],matchesB:[],wasReady:false};
...
function exportComparison(){if(!bothReady())return;const t=tr(),rows=[[t.brand],[t.indicator,name(state.a),t.year,name(state.b),t.year]];['happiness',...Object.keys(DATA.indicatorMeta)].forEach(k=>{const a=metricLatest(state.a,k),b=metricLatest(state.b,k),label=k==='happiness'?t.happiness:meta(k).label;rows.push([label,a?.value??'',a?.year??'',b?.value??'',b?.year??''])});const csv='\ufeff'+rows.map(r=>r.map(v=>'"'+String(v).replaceAll('"','""')+'"').join(',')).join('\n'),blob=new Blob([csv],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`miraah-comparison-${state.a}_${state.b}.csv`;a.click();URL.revokeObjectURL(url)}
init();
'''
...
SEO_HEAD = (
    ...
    f'<meta name="description" content="مرآة — قارن الدول بالأرقام عبر مؤشرات جودة الحياة والاقتصاد والسعادة.">'
    f'<meta name="robots" content="index, follow">'
    f'<meta name="googlebot" content="index, follow">'
    f'<link rel="canonical" href="{CANONICAL_URL}">'
    f'{brand.brand_head_links(social_card_abs=_OG_IMAGE)}'
    f'<meta property="og:type" content="website">'
    f'<meta property="og:url" content="{CANONICAL_URL}">'
    f'<meta property="og:title" content="مرآة | قارن الدول بالأرقام">'
    f'<meta property="og:description" content="مرآة — قارن الدول بالأرقام عبر مؤشرات جودة الحياة والاقتصاد والسعادة.">'
    f'<meta property="og:locale" content="ar_AR">'
    f'<meta property="og:locale:alternate" content="en_US">'
    f'<meta name="twitter:title" content="مرآة | قارن الدول بالأرقام">'
    f'<meta name="twitter:description" content="مرآة — قارن الدول بالأرقام عبر مؤشرات جودة الحياة والاقتصاد والسعادة.">'
    f'<script type="application/ld+json" id="miraah-jsonld">'
    f'{{"@context":"https://schema.org","@type":"WebApplication","name":"مرآة","alternateName":"Mir\u2019ah",'
    f'"url":"{CANONICAL_URL}","description":"مرآة — قارن الدول بالأرقام عبر مؤشرات جودة الحياة والاقتصاد والسعادة.",'
    f'"applicationCategory":"BusinessApplication","operatingSystem":"Any","inLanguage":["ar","en"],'
    f'"image":"{_OG_IMAGE}","logo":"{_LOGO}",'
...
def write_seo_files() -> None:
    ...
    # in scripts/render_passport_pages.py (PASSPORT_INDEXING_ENABLED=True).
...
def main():
    ...
    for cand in ("\nconst MIRAAH_THEME_KEY", "\nconst $="):
        idx = source.find(cand, data_start)
        if idx != -1:
            marker = idx
            break
    if marker is None:
